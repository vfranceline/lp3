...
public class ServidorDespachante extends UnicastRemoteObject implements ServicoDespachante {
    ...
    private void processarMatching(RequisicaoCorrida requisicao) {        
        ...
        try {
            ...
            if (motorista == null){
                log("Não foi possivel encontrar motorista");
            } else {
                String atribuicaoID = UUID.randomUUID().toString().substring(0,8).toUpperCase();

                motorista.setAtribuicaoAtual(atribuicaoID);
                requisicao.setMotoristaAtribuido(motorista.getInfo().getMotoristaId());
                requisicao.setAtribuicaoId(atribuicaoID);         
                
                Atribuicao atribuicao = new Atribuicao(atribuicaoID, requisicao.getCorridaId(), requisicao.getMotoristaAtribuido());

                atribuicoes.put(atribuicaoID, atribuicao);
                log("[MATCHING] Corrida=" + requisicao.getCorridaId() + 
                    " Motorista=" + requisicao.getMotoristaAtribuido() + 
                    " Distancia=" + this.calcularDistancia(motorista.getInfo().getPosicaoAtual(), requisicao.getOrigem()) + 
                    " Tentativa=1");

                notificarMotorista(motorista.getInfo().getMotoristaId(), atribuicaoID);
            }
        } finally {
            lockMatching.unlock();
        }
    ...
    private Motorista encontrarMelhorMotorista(RequisicaoCorrida requisicao) {
        ...
        for (Motorista motorista : motoristas.values()){
            if (motorista.isDisponivel()){
                double distanciaMotorista = this.calcularDistancia(motorista.getInfo().getPosicaoAtual(), requisicao.getOrigem());

                if (melhorMotorista == null){
                    melhorMotorista = motorista;
                    menorDistancia = distanciaMotorista;
                    continue;
                }                

                if (menorDistancia > distanciaMotorista) {
                    melhorMotorista = motorista;
                    menorDistancia = distanciaMotorista;
                } 
                else if (menorDistancia == distanciaMotorista) {
                    long timestamp_melhor = melhorMotorista.getInfo().getTimestamp();
                    long timestamp_motor = motorista.getInfo().getTimestamp();

                    if (timestamp_melhor < timestamp_motor) {
                        melhorMotorista = motorista;
                    }
                }
            }
    ...
    private void notificarMotorista(String motoristaId, String atribuicaoId) {
        ...
        Atribuicao atribuicao = atribuicoes.get(atribuicaoId);
        RequisicaoCorrida corrida = corridas.get(atribuicao.getCorridaId());
        InfoPassageiro infoPassageiro = corrida.getPassageiro();

        AtribuicaoCorrida atribuicaoCorrida = new AtribuicaoCorrida(atribuicaoId, atribuicao.getCorridaId(), infoPassageiro, corrida.getOrigem(), corrida.getDestino(), corrida.getPrioridade(), atribuicao.getTimestampAtribuicao());
        
        Thread foiConfirmada = new Thread (() -> {
        ...
        try {
            ...
            executorTimeout.schedule(foiConfirmada, TIMEOUT_CONFIRMACAO_MS, TimeUnit.MILLISECONDS);
        } catch (RemoteException e) {
            log("ERRO ao notificar motorista: " + e.getMessage());
            tratarFalhaConfirmacao(atribuicaoId);
        }
    }
    ...
    private void tratarFalhaConfirmacao(String atribuicaoId) {
        ...
        if (atribuicao.isConfirmada()){
            ...
            // * 2. Logar falha: "[FALHA] Corrida=X Motorista=Y Motivo=TimeoutConfirmacao Reatribuindo=true"
            log("[FALHA] Corrida=" + corrida.getCorridaId() + 
                    " Motorista=" + atribuicao.getMotoristaId() + 
        ...
        // 4. Buscar corrida E VERIFICAR O STATUS ATUAL
        // (A corrida pode ter sido EXPIRADA ou CANCELADA enquanto esperava)
        if (corrida.getStatus() != StatusRequisicao.PENDENTE) {
            log("REATRIBUICAO CANCELADA - Corrida=" + corrida.getCorridaId() + " Status=" + corrida.getStatus());
            return; // Não reprocessar
        }
        // se ainda tiver pendente, reseta
        // *    - setMotoristaAtribuido(null)
